 ...
 *       checkpoints before throwing anR exception, per job execution attempt. Only relevant if
...
public class DataStreamAllroundTestJobFactory {
    ...
    private static final ConfigOption<Integer> SEQUENCE_GENERATOR_SRC_CHANGING_RATE_INTERVAL =
            ConfigOptions.key("sequence_generator_source.changing_rate_interval")
                    .intType()
                    .defaultValue(60000);

    private static final ConfigOption<Double> SEQUENCE_GENERATOR_SRC_CHANGING_RATE_RATIO =
            ConfigOptions.key("sequence_generator_source.changing_rate_ratio")
                    .doubleType()
                    .defaultValue(0.5);

    private static final ConfigOption<Integer> SEQUENCE_GENERATOR_SRC_CHANGING_RATE_STEPS =
            ConfigOptions.key("sequence_generator_source.changing_rate_steps")
                    .intType()
                    .defaultValue(5);
    ...
    public static SourceFunction<Event> createEventSource(ParameterTool pt) {
        return new SequenceGeneratorSource(
                ...
                pt.getInt(
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_INTERVAL.key(),
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_INTERVAL.defaultValue()),
                pt.getDouble(
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_RATIO.key(),
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_RATIO.defaultValue()),
                pt.getInt(
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_STEPS.key(),
                        SEQUENCE_GENERATOR_SRC_CHANGING_RATE_STEPS.defaultValue())
                );
    }
    ...
    public static WindowedStream<Event, Integer, TimeWindow> applyTumblingWindows(
