...
public class SequenceGeneratorSource extends RichParallelSourceFunction<Event>
    ...
    SequenceGeneratorSource(
            ...
            int changingRateInterval,
            double changingRateRatio,
            int changingRateSteps) {
        ...
        this.changingRateInterval = changingRateInterval;
        this.changingRateRatio = changingRateRatio;
        this.changingRateValue = this.elementsPerSecond * this.changingRateRatio;
        this.changingRateSteps = changingRateSteps;
    }
    ...
    private void runActive(SourceContext<Event> ctx) throws Exception {
        ...
        while (running) {
            ...
            long eventTime =
                    Math.max(0, generateEventTimeWithOutOfOrderness(random, monotonousEventTime));
    ...
    private long generateEventTimeWithOutOfOrderness(Random random, long correctTime) {
        if (maxOutOfOrder > 0) {
            return correctTime
                    - maxOutOfOrder
                    + ((random.nextLong() & Long.MAX_VALUE) % (2 * maxOutOfOrder));
        } else {
            return correctTime;
        }
    }
