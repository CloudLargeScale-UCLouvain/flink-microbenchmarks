...
public class AuctionSourceFunction extends RichParallelSourceFunction<Auction> implements CheckpointedFunction {
    ...
    public AuctionSourceFunction(ParameterTool params) {
        this.rate = params.getDouble("auction-srcRate", 50000);
        this.eventsEmitted = params.getLong("auction-eventsEmitted", Long.MAX_VALUE);
        ...
        LoggerFactory.getLogger(AuctionSourceFunction.class).info(this.toString());
    }
        ...
        while (running && eventsCountSoFar < eventsEmitted) {
            ...
            if (!fixedRate && endTime - lastChangingRate > this.changingRateInterval) {
                if (increasingRate) {
                    ...
                    LoggerFactory.getLogger(AuctionSourceFunction.class).info("Auction: Increasing rate to {}", rate);
                } else {
                    rate -= changingRateValue;
                    LoggerFactory.getLogger(AuctionSourceFunction.class).info("Auction: Decreasing rate to {}", rate);
                }
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        getRuntimeContext()
                .getMetricGroup()
                .gauge("AuctionRate", new Gauge<Double>() {
                    @Override
